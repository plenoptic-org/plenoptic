...
We call synthesize in the same way and can even make use of the original {func}`~plenoptic.plot.synthesis_status` function to see what synthesis looks like

```{code-cell} ipython3
old_mad.synthesize(store_progress=True)
po.plot.synthesis_status(old_mad)
```

```{code-cell} ipython3
new_mad.synthesize(store_progress=True)
po.plot.synthesis_status(new_mad)
```
...
```{code-cell} ipython3
po.plot.synthesis_imshow(new_mad, iteration=10)
```
