...
Once you are familiar with the existing synthesis objects included in `plenoptic`, you may wish to change some aspect of their function. For example, you may wish to change how the {class}`~plenoptic.MADCompetition` initializes the MAD image or alter the objective function of {class}`~plenoptic.Metamer`. While you could certainly start from scratch or copy the source code of the object and alter them directly, an easier way to do so is to create a new sub-class: an object that inherits the synthesis object you wish to modify and over-writes some of its existing methods.

For example, you could create a version of {class}`~plenoptic.MADCompetition` that starts with a different natural image (rather than with `image` argument plus normally-distributed noise) by creating the following object:
...
```{code-cell} ipython3
class MADCompetitionVariant(po.MADCompetition):
...
We can then interact with this new object in the same way as the original {class}`~plenoptic.MADCompetition` object, the only difference being how {func}`~plenoptic.MADCompetition.setup` is called:
...
We call synthesize in the same way and can even make use of the original {func}`~plenoptic.plot.mad_synthesis_status` function to see what synthesis looks like

```{code-cell} ipython3
old_mad.synthesize(store_progress=True)
po.plot.mad_synthesis_status(old_mad, included_plots=["mad_image", "mad_loss"])
```

```{code-cell} ipython3
new_mad.synthesize(store_progress=True)
po.plot.mad_synthesis_status(new_mad, included_plots=["mad_image", "mad_loss"])
```
...
```{code-cell} ipython3
po.plot.mad_image(new_mad, iteration=10)
```
