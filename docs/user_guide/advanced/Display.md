...
We need to tell {func}`~plenoptic.plot.imshow` that the image(s) are RGB in order for it to be plot correctly.

```{code-cell} ipython3
rgb = torch.rand(2, 3, 256, 256)
print(rgb.shape)
fig = po.plot.imshow(rgb, as_rgb=True);
```
...
```{code-cell} ipython3
po.plot.imshow(coeffs[:, 1:-1], batch_idx=0)
po.plot.imshow(coeffs[:, 1:-1], batch_idx=1);
```

We really don't want to interpret those values as RGB.

Note that in the above {func}`~plenoptic.plot.imshow` calls, we had to specify the `batch_idx`. This function expects a 4d tensor, but if it has more than one channel and more than one batch (and it's not RGB), we can't display everything. The user must therefore specify either `batch_idx` or `channel_idx`.

```{code-cell} ipython3
po.plot.imshow(coeffs[:, 1:-1], channel_idx=0);
```
...
# animshow happy
po.plot.animshow(coeffs.unsqueeze(1), batch_idx=0, vrange="indep1")
```
...
model = po.models.OnOff((7, 7)).to(DEVICE)
model.eval()
rep = model(img)
```
...
ps = po.models.PortillaSimoncelli(img.shape[-2:]).to(DEVICE)
rep = ps(img)
po.plot.plot_representation(data=rep);
```
...
```{code-cell} ipython3
po.plot.plot_representation(ps, rep);
```
