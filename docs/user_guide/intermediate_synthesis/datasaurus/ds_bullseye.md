...
In this notebook, we will create a datasaurus metamer shaped like a bullseye: two circles with the same center and different radii.
...
The following plot shows the `bullseye` dataset from the original datasaurus dozen, along with its representation.
...
Our intended shape here, as can be seen above, is two circles with identical centers in the middle of the screen and different radii. To encourage metamer synthesis to find such a dataset, we create two functions:
- `circle_penalty` computes the distance of each point to some (user-specified) center point and returns the mean-squared error between that distance and a user-specified radius.
- `bullseye_penalty` arbitrarily splits the points in half and calls `circle_penalty` twice, once for each data split and radius, returning the sum of these two penalties.

To use this penalty with synthesis, we define the center and radii (try changing these to different values!) and combine the resulting value with a range penalty which requires all points to lie between 0 and 100.
...
def penalty(x):
    ...
    bullseye = bullseye_penalty(x, ctr, radii)
...
for rad in radii:
    xs = np.linspace(ctr[0] - rad, ctr[0] + rad)
    axes[0].plot(xs, ctr[1] + np.sqrt(rad**2 - (xs - ctr[0]) ** 2), "k--", zorder=0)
    axes[0].plot(xs, ctr[1] - np.sqrt(rad**2 - (xs - ctr[0]) ** 2), "k--", zorder=0)
...
In the video of the synthesis above, we can see the dataset shifting itself so that it becomes a bullseye and a datasaurus metamer, at roughly the same rate. Unlike [](ds_circle.md), here synthesis is able to satisfy both the metamer and penalty objectives, placing the points on the target circles while also matching the target statistics.
...
cached_met = po.data.fetch_data("datasaurus_metamers.tar.gz") / "datasaurus-bullseye.pt"
