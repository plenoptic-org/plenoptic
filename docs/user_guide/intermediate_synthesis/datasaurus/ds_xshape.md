...
In this notebook, we will create a datasaurus metamer shaped like an X.
...
The following plot shows the `x_shape` dataset from the original datasaurus dozen, along with its representation.
...
Our intended shape here, as can be seen above, is two lines crossing in the center of the scatter plot. To encourage metamer synthesis to find such a dataset, we create several functions (all of these show up in other notebooks in this series and so may look familiar):
- `predict_line` returns the y-values for a user-defined line, evaluated at the specified x-values.
- `lines_penalty` returns the mean-squared error for a set of points, given a user-specified set of slopes and intercepts.
- `slant_penalty` ensures that the input arguments are the proper type and shape, then calls `lines_penalty`.

Note that we arbitrarily split the points in two, assigning the first half to one line and the second half to the other, regardless of where they started.

To use this penalty with synthesis, we define the slopes and intercepts and combine the resulting value with a range penalty which requires all points to lie between 0 and 100. In order for the penalty to find an X shape, there must be two lines: one with a positive slope and a negative intercept, the other with a negative slope and a large positive intercept.
...
# Both these variables should have the same number of values. Try changing them to other
# values!
slopes = [1.5, -1.5]
intercepts = [-30, 130]
...
def penalty(x):
    ...
    xshape = slant_penalty(x, slopes, intercepts)
...
xs = np.asarray([0, 100])
for sl, inter in zip(slopes, intercepts):
    axes[0].plot(xs, predict_line(xs, inter, sl), "k--", zorder=0)
...
if os.environ.get("DATASAURUS_CHECK", False):
    ...
    cached_met = (
        po.data.fetch_data("datasaurus_metamers.tar.gz") / "datasaurus-xshape.pt"
    )
