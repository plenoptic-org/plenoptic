...
Download the executed notebook: **{nb-download}`ds_slantdown.ipynb`**!

Run it in your browser: **{binder}`ds_slantdown.ipynb`**!

:::

# slantdown

In this notebook, we will create a datasaurus metamer shaped like multiple lines running from the top left to the bottom right.
...
The following plot shows the `slant_down` dataset from the original datasaurus dozen, along with its representation.
...
Our intended shape here, as can be seen above, is five parallel lines with a negative slope, roughly evenly distributed across the plot. To encourage metamer synthesis to find such a dataset, we create several functions (all of these show up in other notebooks in this series and so may look familiar):
...
cached_met = (
    po.data.fetch_data("datasaurus_metamers.tar.gz") / "datasaurus-slantdown.pt"
)
