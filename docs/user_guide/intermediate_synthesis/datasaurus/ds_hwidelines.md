...
In this notebook, we will create a datasaurus metamer shaped like two tall horizontal lines.
...
The following plot shows the `high_lines` dataset from the original datasaurus dozen, along with its representation.
...
Our intended shape here, as can be seen above, is two horizontal streaks, about 20 high and 60 to 80 wide. To encourage metamer synthesis to find such a dataset, we create several functions (all of these show up in other notebooks in this series and so may look familiar):
...
- `hwidelines_penalty` ensures that the input arguments are the proper type and shape, then calls `widelines_penalty`.
...
def widelines_penalty(data, intercepts, slope, margin):
    ...
    for i, (inter, sl) in enumerate(zip(intercepts, slope)):
        ...
        # instead of just taking the error
...
def penalty(x):
    ...
    lines = hwidelines_penalty(x, y_vals, margin)
...
for y in y_vals:
    axes[0].fill_between(
        np.asarray([0, 100]), y - margin, y + margin, zorder=0, alpha=0.2, color="C0"
    )
