...
In this notebook, we will create a datasaurus metamer not present in the original set, encouraging the dots to group themselves into little regular polygons.
...
The penalty in this example was developed while working on the [](ds_hwidelines.md) and [](ds_vwidelines.md) examples (you can see it also being used there). The goal for this penalty was to encourage the points to distribute themselves a little more evenly across the plot, instead of clumping up on top of each other. To that end, the penalty breaks the points into small groups and computes the pairwise Euclidean distance between all points in each group, and then computes the MSE between these distances and some specified target. Importantly, this penalty does not place any restriction on the distances between groups (see [](ds_oval.md) for what adding that restriction looks like). Additionally, because the total number of points may not be divisible by the group size, some small number of points are completely unconstrained and can fly to any part of the plot.

To use this penalty with synthesis, we define the size of these groups and the target distance (try changing these to different values!) and combine the resulting value with a range penalty which requires all points to lie between 0 and 100.
...
def polygon_penalty(data, target_dist, nbr):
    ...
    # break data into groups of size nbr
    pts = einops.rearrange(
        data[..., : nbr * (data.shape[-1] // nbr)], "d (n1 n2) -> n1 n2 d", n2=nbr
    )
    # compute pairwise distance between all the points in each group
    dist = torch.cdist(pts, pts)
    # Grab the lower triangle indices...
    tril_idx = torch.tril_indices(pts.shape[1], pts.shape[1], -1)
    # ... so we can throw away redundant values
    dist = dist[:, tril_idx[0], tril_idx[1]]
    # And then compute the MSE between these distances and the target
...
def penalty(x):
    ...
    polygon = polygon_penalty(x, target_distance, neighborhood_size)
...
if os.environ.get("DATASAURUS_CHECK", False):
    ...
    cached_met = (
        po.data.fetch_data("datasaurus_metamers.tar.gz") / "datasaurus-polygons.pt"
    )
