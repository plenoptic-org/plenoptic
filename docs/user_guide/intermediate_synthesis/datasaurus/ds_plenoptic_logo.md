...
In this notebook, we will create a datasaurus metamer starting from the plenoptic logo. Unlike the other examples, we will not add any penalty to encourage any particular shape. See [](datasaurus-index) for an overview of the datasaurus dozen dataset.
