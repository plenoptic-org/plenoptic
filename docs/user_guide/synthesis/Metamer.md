...
For this purpose, the {class}`~plenoptic.synthesize.metamer.Metamer` class takes
an optional {attr}`~plenoptic.synthesize.metamer.Metamer.penalty_function`
argument at initialization. The
{attr}`~plenoptic.synthesize.metamer.Metamer.penalty_function` is a callable
that takes as an input the synthesized metamer image, and returns a scalar
penalty. This scalar penalty is added to the loss during optimization, and it
can be used to control certain properties of the synthesized metamer.

For example, the default
{attr}`~plenoptic.synthesize.metamer.Metamer.penalty_function` uses the
{func}`~plenoptic.tools.regularization.penalize_range` function to penalize
pixel values that fall outside the range [0, 1], helping to keep the synthesized
...
argument `allowed_range` in the
{func}`~plenoptic.tools.regularization.penalize_range` function to define a new
range penalization. Below we show how to constrain the pixel range to be between
0.2 and 0.8.
...
{attr}`~plenoptic.synthesize.metamer.Metamer.penalties` attribute, analogous to
the {attr}`~plenoptic.synthesize.metamer.Metamer.losses` attribute:
