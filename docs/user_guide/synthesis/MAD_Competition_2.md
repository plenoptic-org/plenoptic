...
In `plenoptic`, a single instantiation of {class}`~plenoptic.MADCompetition` synthesizes a single image, holding the {attr}`~plenoptic.MADCompetition.reference_metric` constant while either maximizing or minimizing {attr}`~plenoptic.MADCompetition.optimized_metric`, depending on the value of the {attr}`~plenoptic.MADCompetition.minmax` argument. A full set of MAD competition images consists of four images, maximizing and minimizing each of the two metrics. For each pair of images, one metric predicts they are perceptually identical, while the other metric predicts they are as dissimilar as possible. This set therefore allows us to efficiently compare the two metrics.
...
- $t$ is 1 if {attr}`~plenoptic.MADCompetition.minmax` is `'min'` and -1 if it's `'max'`
- $L_1$ is {attr}`~plenoptic.MADCompetition.optimized_metric`
- $L_2$ is {attr}`~plenoptic.MADCompetition.reference_metric`
...
- $\lambda_2$ is {attr}`~plenoptic.MADCompetition.range_penalty_lambda`
...
Note that, as described in the beginning of this notebook, we synthesize these images by optimizing a tradeoff between the loss of these two metrics, weighted by the {attr}`~plenoptic.MADCompetition.metric_tradeoff_lambda`. If that argument is unset, we default to something we think is reasonable, but in practice, we often need to experiment and find the appropriate value, trying out different values until the fixed metric stays constant while the synthesis metric decreases or increases as desired.
...
At the most basic, all we need to do is call the {func}`~plenoptic.MADCompetition.synthesize` method. Let's do that and then view the outcome. There are several additional arguments to {func}`~plenoptic.MADCompetition.synthesize` but none are required.
...
As described in the opening paragraph, a full set of MAD competition synthesized images consists of four images. In order to create the other images, we must create a new instance of {class}`~plenoptic.MADCompetition`. Let's do that for the other images now:
...
To continue synthesis, we can simply call {func}`~plenoptic.MADCompetition.synthesize` again, and we then pick up right where we left off.
...
In general, finding an appropriate value for {attr}`~plenoptic.MADCompetition.metric_tradeoff_lambda` will require both some consideration on the part of the user and some testing of different values.
