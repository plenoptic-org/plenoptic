...
See [Issue #365](https://github.com/plenoptic-org/plenoptic/issues/365) for an example of how we worked through finding a good optimization configuration for the {class}`~plenoptic.models.PortillaSimoncelli` model.
...
- In the middle: normalize statistics based on their value in the original image (note: not the image the model is taking as input! this will likely make optimization very difficult). The {func}`~plenoptic.optim.groupwise_relative_l2_norm_factory` function will perform this when computing the loss, and allows for further tweaking.
...
It is possible that, even after tweaking your model and trying a variety of different optimization configurations, your optimization is still converging with a weirdly high loss. One possible reason for this is that different parts of your representation are trading off with each other, so that it's hard to reduce error in one part of your representation without increasing it in another. In order to determine whether this is the case, it can be helpful to use the {class}`~plenoptic.MetamerCTF` class, optimizing different components of the representation independently and then together (see [](metamer-coarse-to-fine) for more details about {class}`~plenoptic.MetamerCTF`; this is equivalent to but perhaps more convenient than creating individual sub-models which return the different parts of the representation). If the loss is able to decrease with each component separately, but increases again when optimizing all of them together, your statistics are likely trading off against each other. If this is the case, then you can try removing the offending parts of your representation or reweighting them (see [](ps-loss-function)).

See the discussion of the variance of the highpass residuals and the min/max in [Issue #365](https://github.com/plenoptic-org/plenoptic/issues/365) for an example of how we worked through similar problems for the {class}`~plenoptic.models.PortillaSimoncelli` model.
...
- The coarse-to-fine optimization, discussed in the [metamer notebook](metamer-coarse-to-fine), is an example of changing optimization by extending the {class}`~plenoptic.Metamer` class.
