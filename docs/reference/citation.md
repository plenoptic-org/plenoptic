...
- {class}`~plenoptic.Metamer` or {class}`~plenoptic.MetamerCTF`: {cite:alp}`Portilla2000-param-textur`.
- {class}`~plenoptic.MADCompetition`: {cite:alp}`Wang2008-maxim-differ`.
- {class}`~plenoptic.Eigendistortion`: {cite:alp}`Berardino2017-eigen`.
- {class}`~plenoptic.model_components.SteerablePyramidFreq`: {cite:alp}`Simoncelli1995-steer-pyram` ({cite:alp}`Simoncelli1992-shift-multi` contains a longer discussion about the motivation and the logic, while {cite:alp}`Simoncelli1995-steer-pyram` describes the implementation that is used here).
