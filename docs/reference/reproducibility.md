...
- {class}`~plenoptic.synthesize.metamer.Metamer` and {class}`~plenoptic.synthesize.mad_competition.MADCompetition` APIs were changed, replacing the range penalty with a more generic {attr}`~plenoptic.synthesize.metamer.Metamer.penalty_function`. See [](metamer-regularization) for more details on how to use this new capability.
