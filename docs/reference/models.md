...
{class}`~plenoptic.MADCompetition` uses metrics, rather than models, which have the following requirements (use the {func}`~plenoptic.validate.validate_metric` function to check whether your metric meets the following requirements and see [](metrics-api) for some examples):
