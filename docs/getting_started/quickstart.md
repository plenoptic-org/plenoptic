...
Download the executed notebook: **{nb-download}`quickstart.ipynb`**!

Run it in your browser: **{binder}`quickstart.ipynb`**!
...
```{code-cell} ipython3
fig = po.plot.imshow(torch.cat([im, rep]), title=["Original image", "Model output"])
```
