...
`plenoptic` is also built on top of [pyrtools](https://pyrtools.readthedocs.io/en/latest/). `pyrtools` includes `numpy` implementations of the [image pyramids](https://docs.plenoptic.org/docs/branch/main/api/components.html) found in plenoptic, as well as other image-processing tools.
...
- If you would like to run the jupyter notebooks locally: `pip install "plenoptic[nb]"` or `pip install -e ".[nb]"`. This includes `torchvision` (which has some models we'd like to use), `jupyter`, and related libraries. See [](jupyter-doc) for a discussion of several ways to handle jupyter and python virtual environments. Note that you can run our notebooks in the cloud using Binder, no installation required! Just click on the Binder logo in the top right or in the link on the top of each notebook.
