...
- the initialization method must accept `range_penalty_lambda: float` and `allowed_range: Tuple[float, float]` arguments, which should be used with {func}`~plenoptic.optim.penalize_range` to constrain the range of synthesis output.
