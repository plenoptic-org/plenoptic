...
def get_portilla_simoncelli_synthesize_filename(torch_version=None):
    ...
    # strings
    if (
        version.parse(torch_version) < version.parse("1.12")
        or DEVICE.type == "cuda"
    ):
        torch_version = ""
    ...
    # quality)
    name_template = (
        "portilla_simoncelli_synthesize{gpu}{torch_version}_ps-refactor.npz"
    )
...
class TestFilters:
    ...
    @pytest.mark.parametrize("std", [1.0, 20.0, 0.0])
