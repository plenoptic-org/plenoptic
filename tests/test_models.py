...
class TestLinear(object):
    ...
    def test_linear_metamer(self):
        model = po.simul.Linear()
        image = plt.imread(op.join(DATA_DIR, "nuts.pgm")).astype(float) / 255.0
        im0 = (
            torch.tensor(image, requires_grad=True, dtype=DTYPE)
            .squeeze()
            .unsqueeze(0)
            .unsqueeze(0)
        )
        M = po.synth.Metamer(im0, model)
        matched_image, matched_representation = M.synthesize(
            max_iter=3, learning_rate=1, seed=1
        )
...
class TestLinearNonlinear(object):
    ...
    def test_linear_nonlinear_metamer(self):
        model = po.simul.Linear_Nonlinear()
        image = plt.imread(op.join(DATA_DIR, "metal.pgm")).astype(float) / 255.0
        im0 = (
            torch.tensor(image, requires_grad=True, dtype=torch.float32)
            .squeeze()
            .unsqueeze(0)
            .unsqueeze(0)
        )
        M = po.synth.Metamer(im0, model)
        matched_image, matched_representation = M.synthesize(
            max_iter=3, learning_rate=1, seed=0
        )
...
class TestPooling(object):
    def test_creation(self):
        ang_windows, ecc_windows = po.simul.pooling.create_pooling_windows(
            0.87, (256, 256)
        )

    def test_creation_args(self):
        ang, ecc = po.simul.pooling.create_pooling_windows(
            0.87, (100, 100), 0.2, 30, 1.2, 0.7
        )
        ang, ecc = po.simul.pooling.create_pooling_windows(
            0.87, (100, 100), 0.2, 30, 1.2, 0.5
        )
    ...
    def test_ecc_windows(self):
        ...
        windows = po.simul.pooling.log_eccentricity_windows(
            (256, 256), window_spacing=0.5
        )
        windows = po.simul.pooling.log_eccentricity_windows(
            (256, 256), window_spacing=1
        )
    ...
    def test_calculations(self):
        ...
        assert (
            po.simul.pooling.calc_eccentricity_window_spacing(n_windows=4)
            == 0.8502993454155389
        )
        assert (
            po.simul.pooling.calc_eccentricity_window_spacing(scaling=0.87)
            == 0.8446653390527211
        )
        assert (
            po.simul.pooling.calc_eccentricity_window_spacing(5, 10, scaling=0.87)
            == 0.8446653390527211
        )
        assert (
            po.simul.pooling.calc_eccentricity_window_spacing(5, 10, n_windows=4)
            == 0.1732867951399864
        )
        assert po.simul.pooling.calc_eccentricity_n_windows(0.8502993454155389) == 4
        assert (
            po.simul.pooling.calc_eccentricity_n_windows(0.1732867951399864, 5, 10) == 4
        )
    ...
    def test_PoolingWindows_cosine(self, num_scales, transition_region_width):
        ...
        pw = po.simul.pooling.PoolingWindows(
            0.5,
            im.shape[2:],
        ...
        pw(im)
        with pytest.raises(Exception):
            po.simul.PoolingWindows(0.2, (64, 64), 0.5)
    ...
    def test_PoolingWindows(self, num_scales):
        ...
        with pytest.raises(Exception):
            po.simul.pooling.PoolingWindows(
                0.5,
                im.shape[2:],
                num_scales=num_scales,
    ...
    def test_PoolingWindows_project(self):
        im = plt.imread(op.join(DATA_DIR, "nuts.pgm"))
        im = torch.tensor(im, dtype=DTYPE, device=DEVICE).unsqueeze(0).unsqueeze(0)
        pw = po.simul.pooling.PoolingWindows(0.5, im.shape[2:])
        pw = pw.to(DEVICE)
        pooled = pw(im)
        pw.project(pooled)
        pw = po.simul.pooling.PoolingWindows(0.5, im.shape[2:], num_scales=3)
        pw = pw.to(DEVICE)
        pooled = pw(im)
        pw.project(pooled)
    ...
    def test_PoolingWindows_nonsquare(self):
        ...
        for sh in [(256, 128), (256, 127), (256, 125), (125, 125), (127, 125)]:
            ...
            rgc = po.simul.PooledRGC(0.9, tmp.shape[2:])
            rgc = rgc.to(DEVICE)
            rgc(tmp)
            v1 = po.simul.PooledRGC(0.9, tmp.shape[2:])
            v1 = v1.to(DEVICE)
            v1(tmp)
    ...
    def test_PoolingWindows_plotting(self):
        im = plt.imread(op.join(DATA_DIR, "nuts.pgm"))
        im = torch.tensor(im, dtype=DTYPE, device=DEVICE)
        pw = po.simul.PoolingWindows(0.8, im.shape, num_scales=2)
        pw = pw.to(DEVICE)
    ...
    def test_PoolingWindows_caching(self, tmp_path):
        ...
        pw = po.simul.PoolingWindows(0.8, im.shape, num_scales=2, cache_dir=tmp_path)
        pw = po.simul.PoolingWindows(0.8, im.shape, num_scales=2, cache_dir=tmp_path)
    ...
    def test_PoolingWindows_parallel(self, tmp_path):
        if torch.cuda.device_count() > 1:
            ...
            pw(im)
            pw = po.simul.pooling.PoolingWindows(
                0.5, im.shape[2:], transition_region_width=1
            )
            pw = pw.parallel(devices)
            pw(im)
            for sh in [(256, 128), (256, 127), (256, 125), (125, 125), (127, 125)]:
                ...
                v1(tmp)
            pw = po.simul.PoolingWindows(0.8, im.shape[2:], num_scales=2)
            pw = pw.parallel(devices)
            pw.plot_window_areas()
            pw.plot_window_widths()
            for i in range(2):
...
class TestPooledVentralStream(object):
    def test_rgc(self):
        im = plt.imread(op.join(DATA_DIR, "nuts.pgm"))
        im = torch.tensor(im, dtype=DTYPE, device=DEVICE).unsqueeze(0).unsqueeze(0)
        rgc = po.simul.PooledRGC(0.5, im.shape[2:])
        rgc = rgc.to(DEVICE)
        rgc(im)
    ...
    def test_rgc_2(self):
        im = plt.imread(op.join(DATA_DIR, "nuts.pgm"))
        im = torch.tensor(im, dtype=DTYPE, device=DEVICE).unsqueeze(0).unsqueeze(0)
        rgc = po.simul.PooledRGC(0.5, im.shape[2:], transition_region_width=1)
        rgc = rgc.to(DEVICE)
        rgc(im)
        _ = rgc.plot_window_widths("degrees")
        _ = rgc.plot_window_widths("degrees", jitter=0)
    ...
    def test_rgc_save_load(self, tmp_path):
        ...
        rgc = po.simul.PooledRGC(0.5, im.shape[2:], cache_dir=tmp_path)
        rgc = rgc.to(DEVICE)
        rgc(im)
        rgc.save_reduced(op.join(tmp_path, "test_rgc_save_load.pt"))
        rgc_copy = po.simul.PooledRGC.load_reduced(
            op.join(tmp_path, "test_rgc_save_load.pt")
        )
        rgc_copy = rgc_copy.to(DEVICE)
        if not len(rgc.PoolingWindows.angle_windows) == len(
            rgc_copy.PoolingWindows.angle_windows
        ):
            raise Exception(
                "Something went wrong saving and loading, the lists of angle windows"
                " are not the same length!"
            )
        if not len(rgc.PoolingWindows.ecc_windows) == len(
            rgc_copy.PoolingWindows.ecc_windows
        ):
            raise Exception(
                "Something went wrong saving and loading, the lists of ecc windows"
                " are not the same length!"
            )
        ...
        for i in range(len(rgc.PoolingWindows.angle_windows)):
            if not rgc.PoolingWindows.angle_windows[i].allclose(
                rgc_copy.PoolingWindows.angle_windows[i]
            ):
                raise Exception(
                    "Something went wrong saving and loading, the angle_windows %d are"
                    " not identical!" % i
                )
        for i in range(len(rgc.PoolingWindows.ecc_windows)):
            if not rgc.PoolingWindows.ecc_windows[i].allclose(
                rgc_copy.PoolingWindows.ecc_windows[i]
            ):
                raise Exception(
                    "Something went wrong saving and loading, the ecc_windows %d are"
                    " not identical!" % i
                )
    ...
    def test_frontend_PoolingWindows(self):
        ...
        im = torch.tensor(im, dtype=DTYPE, device=DEVICE).unsqueeze(0).unsqueeze(0)
        frontend = po.simul.Front_End()
        pw = po.simul.PoolingWindows(0.5, (256, 256))
        pw(frontend(im))
    ...
    def test_v1(self):
        im = plt.imread(op.join(DATA_DIR, "nuts.pgm"))
        im = torch.tensor(im, dtype=DTYPE, device=DEVICE).unsqueeze(0).unsqueeze(0)
        v1 = po.simul.PooledV1(0.5, im.shape[2:])
        v1 = v1.to(DEVICE)
        v1(im)
    ...
    def test_v1_norm(self):
        ...
        v1 = po.simul.PooledV1(0.5, im.shape[2:], normalize_dict=stats)
        v1 = v1.to(DEVICE)
        v1(im)
        _ = v1.plot_window_widths("pixels")
        _ = v1.plot_window_areas("pixels")
    ...
    def test_v1_parallel(self):
        if torch.cuda.device_count() > 1:
            ...
            im = plt.imread(op.join(DATA_DIR, "nuts.pgm"))
            im = torch.tensor(im, dtype=DTYPE, device=DEVICE).unsqueeze(0).unsqueeze(0)
            v1 = po.simul.PooledV1(0.5, im.shape[2:]).to(DEVICE)
            v1 = v1.parallel(devices)
    ...
    def test_v1_2(self):
        im = plt.imread(op.join(DATA_DIR, "nuts.pgm"))
        im = torch.tensor(im, dtype=DTYPE, device=DEVICE).unsqueeze(0).unsqueeze(0)
        v1 = po.simul.PooledV1(0.5, im.shape[2:], transition_region_width=1)
        v1 = v1.to(DEVICE)
        v1(im)
        _ = v1.plot_window_widths("pixels")
        _ = v1.plot_window_areas("pixels")
    ...
    def test_v1_mean_luminance(self):
        for fname in ["nuts", "einstein"]:
            ...
            if not torch.allclose(rgc.representation, v1.mean_luminance):
                raise Exception(
                    "Somehow RGC and V1 mean luminance representations are not the "
                    "same for image %s!" % fname
                )
            if not torch.allclose(rgc_rep, v1_rep[..., -rgc_rep.shape[-1] :]):
                raise Exception(
                    "Somehow V1's representation does not have the mean luminance "
    ...
    def test_v1_save_load(self, tmp_path):
        ...
        # first time we cache the windows...
        v1 = po.simul.PooledV1(0.5, im.shape[2:], cache_dir=tmp_path)
        v1 = v1.to(DEVICE)
        v1(im)
        v1.save_reduced(op.join(tmp_path, "test_v1_save_load.pt"))
        v1_copy = po.simul.PooledV1.load_reduced(
            op.join(tmp_path, "test_v1_save_load.pt")
        )
        v1_copy = v1_copy.to(DEVICE)
        if not len(v1.PoolingWindows.angle_windows) == len(
            v1_copy.PoolingWindows.angle_windows
        ):
            raise Exception(
                "Something went wrong saving and loading, the lists of angle windows"
                " are not the same length!"
            )
        if not len(v1.PoolingWindows.ecc_windows) == len(
            v1_copy.PoolingWindows.ecc_windows
        ):
            raise Exception(
                "Something went wrong saving and loading, the lists of ecc windows"
                " are not the same length!"
        ...
        for i in range(len(v1.PoolingWindows.angle_windows)):
            if not v1.PoolingWindows.angle_windows[i].allclose(
                v1_copy.PoolingWindows.angle_windows[i]
            ):
                raise Exception(
                    "Something went wrong saving and loading, the angle_windows %d are"
                    " not identical!" % i
        ...
        for i in range(len(v1.PoolingWindows.ecc_windows)):
            if not v1.PoolingWindows.ecc_windows[i].allclose(
                v1_copy.PoolingWindows.ecc_windows[i]
            ):
                raise Exception(
                    "Something went wrong saving and loading, the ecc_windows %d are"
                    " not identical!" % i
    ...
    def test_v1_metamer(self):
        im = plt.imread(op.join(DATA_DIR, "nuts.pgm"))
        im = (
            torch.tensor(im / 255, dtype=DTYPE, device=DEVICE).unsqueeze(0).unsqueeze(0)
        )
        v1 = po.simul.PooledV1(0.5, im.shape[2:])
        v1 = v1.to(DEVICE)
        metamer = po.synth.Metamer(im, v1)
    ...
    def test_cone_nonlinear(self):
        ...
        # power is different
        (
            v1_lin.representation["mean_luminance"]
            != v1.representation["mean_luminance"]
        ).all()
        # similarly, the representations should be different if cone
        # power is different
...
class TestPortillaSimoncelli(object):
    ...
    def test_portilla_simoncelli(
        ...
        ps = po.simul.Portilla_Simoncelli(
            x.shape[-2:],
            n_scales=n_scales,
    ...
    def test_ps_synthesis(self):
        ...
        n=256
        im0 = torch.Tensor(im)
        model = po.simul.Portilla_Simoncelli(
            [n,n],
            n_scales=4, 
            n_orientations=4, 
        ...
        np.testing.assert_allclose(
            output[0].squeeze().detach().numpy(), im_synth.squeeze(), rtol=1e-3, atol=1e-3
        )
