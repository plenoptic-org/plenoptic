...
def plot_MAD_results(original_image, noise_levels=None, results_dir='~/Documents/MAD_Competition/data/results',
    ...
    if vrange.startswith('row'):
        ...
        for i in range(len(images)//5):
            vr, cmap = pt.tools.display.colormap_range(images[5*i:5*(i+1)],
                                                       vrange.replace('row', 'auto'))
    ...
    fig = pt.tools.display.make_figure(len(images)//5, 5, [zoom*i+1 for i in images.shape[-2:]],
                                       vert_pct=.75)
    for img, ax, t, vr, s in zip(images, fig.axes, titles, vrange_list, super_titles):
        ...
        if s is not None:
            font = {k.replace('_', ''): v for k, v in
                    ax.title.get_font_properties().__dict__.items()}
            # these are the acceptable keys for the fontdict below
            font = {k: v for k, v in font.items() if k in ['family', 'color', 'weight', 'size',
                                                           'style']}
            ...
            fig.text(1+(5/img_size[0]), (1/.75), s, fontdict=font,
