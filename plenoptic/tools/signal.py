...
def rotate_image(img, angle, X=None, Y=None):
    ...
    NOTE: This only works for square images and will raise an exception
    otherwise. ``flip_image`` will work with non-square images but these
    two are not identical (``rotate_image(img, 180)`` and
    ``flip_image(img, True, True)`` are the same, as is the trivial case
    ``rotate_image(img, 0)`` and ``flip_image(img, False, False)``, but
    no others)
...
def crop_image_centered(img, size):
    ...
    TODO: remove and use built-in center_crop
    """
    # clone the tensor so we return a different pointer -- otherwise changes to
    # the original image will influence changes to the new one, and vice versa
    img = img.clone()
    ...
    if crop_amt[1] == 0:
        crop_lims[1] = (0, None)
    return img[..., crop_lims[0][0]:crop_lims[0][1], crop_lims[1][0]:crop_lims[1][1]]
...
def flip_image(img, horizontal=False, vertical=False, X=None, Y=None):
    ...
    ----------
    img : torch.Tensor
        the tensor to rotate. we flip along the last two dimensions
    horizontal, vertical : bool, optional
        whether to flip the image horizontally or vertically,
        respectively
    X, Y : torch.Tensor or None, optional
        either the meshgrid that gives the indices in img (generated by
        calling ``torch.meshgrid(torch.arange(img.shape[-2]),
    ...
    TODO: remove and rely on torch.fliplr torch.flipup
    """
    # clone the tensor so we return a different pointer -- otherwise changes to
    # the original image will influence changes to the new one, and vice versa
    img = img.clone()
