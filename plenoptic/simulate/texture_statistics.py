...
class Texture_Statistics(nn.Module):
    ...
    def forward(self, im0, height='auto', order=3):
        ...
        # initialize pyramid
        # TODO move into init
        self.image_shape = im0.shape[-2:]
        self.pyr = SteerablePyramidFreqTorch(self.image_shape, height=height, order=order, is_complex=True, store_unoriented_bands=True, return_list=True)
        self.n_scales = self.pyr.num_scales
        self.n_orientations = self.pyr.num_orientations

        # get pyramid coefficients
        pyr0 = self.pyr.forward(im0)

        # pyr0 = [pyr[-1]]
        # for i in range(self.n_scales-1, -1, -1):
        #     for j in range(0, self.n_orientations):
        #         pyr0.append(pyr[i+1][j])
        # pyr0.append(pyr[0])
        ...
        #######
        mpyrM = SteerablePyramidFreqTorch(ch.shape[-2:], height=0, order=0, return_list=True)
        ...
        # stats = torch.cat((magMeans0.flatten(),statg0.flatten()))

        stats = torch.cat((statg0.flatten(), magMeans0.flatten(), ace.flatten(), skew0p.flatten(), kurt0p.flatten(), acr.flatten(), C0.flatten(), Cx0.flatten(), Cr0.flatten(), Crx0.flatten(), vHPR0.unsqueeze(0)))
