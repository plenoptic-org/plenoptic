...
REGISTRY_URLS = {
    ...
    "portilla_simoncelli_matlab_test_vectors.tar.gz": OSF_TEMPLATE.format(
        "qtn5y"
    ),
    ...
    "portilla_simoncelli_synthesize_torch_v1.12.0.npz": OSF_TEMPLATE.format(
        "gbv8e"
    ),
    ...
    "portilla_simoncelli_test_vectors_refactor.tar.gz": OSF_TEMPLATE.format(
        "ca7qt"
    ),
    "portilla_simoncelli_synthesize_torch_v1.12.0_ps-refactor.npz": OSF_TEMPLATE.format(
        "vmwzd"
    ),
    "portilla_simoncelli_synthesize_gpu_ps-refactor.npz": OSF_TEMPLATE.format(
        "mqs6y"
    ),
...
def fetch_data(dataset_name: str) -> pathlib.Path:
    ...
    fname = retriever.fetch(
        dataset_name, progressbar=True, processor=processor
    )
    if dataset_name.endswith(".tar.gz"):
