...
class MADCompetition(OptimizedSynthesis):
    ...
    def objective_function(
        self, mad_image: Optional[Tensor] = None, image: Optional[Tensor] = None
    ) -> Tensor:
    ...
    def load(
        self, file_path: str, map_location: Optional[None] = None, **pickle_load_args
    ):
...
def _setup_synthesis_fig(
    ...
    included_plots: List[str] = ["display_mad_image", "plot_loss", "plot_pixel_values"],
    ...
    if fig is None:
        ...
        fig, axes = plt.subplots(
            1, n_subplots, figsize=figsize, gridspec_kw={"width_ratios": width_ratios}
        )
        if n_subplots == 1:
            axes = [axes]
    else:
